...
def process_transmission(UE_device, AP, current_time):
    if UE_device.NLOS == 1:
        ...
        pass  # No action for the LoS case yet.
    else:
...
def plot_room(room, UE_list, signals, mirror, num_mirror, AP, current_sector):
    ...
    for i in range(num_mirror):
        plt.plot(
            [mirror[i].xCorP1, mirror[i].xCorP2],
            [mirror[i].yCorP1, mirror[i].yCorP2],
            "orange",
            label="Mirror: " + str(i),
        )
    ...
    for ue_device in UE_list:
        ...
        if ue_device.NLOS == 1:
            ...
            reflection_distance = math.sqrt(
                (ue_device.xCor - ue_device.reflect_x[indexer]) ** 2
                + (ue_device.yCor - ue_device.reflect_y[indexer]) ** 2
            )
            w_z = w_0 * (1 + reflection_distance / z) ** 0.5
            total_distance = math.sqrt(
                (ue_device.xCor - ue_device.reflect_x[indexer]) ** 2
                + (ue_device.yCor - ue_device.reflect_y[indexer]) ** 2
            ) + math.sqrt((ue_device.reflect_x[indexer] - 0) ** 2 + (ue_device.reflect_y[indexer] - 0) ** 2)
            w_z = w_0 * (1 + total_distance / z) ** 0.5

            plt.plot(
                [ue_device.xCor, ue_device.reflect_x[indexer]], [ue_device.yCor, ue_device.reflect_y[indexer]], "green"
            )
            plt.plot(
                [ue_device.xCor, ue_device.reflect_x[indexer] + w_z / 2],
                [ue_device.yCor, ue_device.reflect_y[indexer]],
                "green",
            )
            plt.plot(
                [ue_device.xCor, ue_device.reflect_x[indexer] - w_z / 2],
            ...
            plt.title(
                ...
                + ", Data Rate = "
                + str(data_rate)
                + " Gbps, Power Recieved [dBM] = "
                + str(p_r)
            )

    plt.plot(
        [0, AP.sector_leftBoundary[current_sector - 1][0] * (room.length * 0.9)],
        [0, AP.sector_leftBoundary[current_sector - 1][1] * (room.length * 0.9)],
        color="b",
    )
    plt.plot(
        [0, AP.sector_rightBoundary[current_sector - 1][0] * (room.length * 0.9)],
        [0, AP.sector_rightBoundary[current_sector - 1][1] * (room.length * 0.9)],
        color="b",
    )
    plt.legend()
...
for i in range(0, len(time)):
    ...
    plt = plot_room(
        simulation_room,
        [UE_snaps[i]],
        [slope_reflection, slope_incident, intercept_reflection, intercept_incident],
        mirrors,
        1,
        AP,
        int(sector[i]),
