...
class MAC_Controller:
    ...
    def setupMAC(
        ...
        if self.mac_protocol == constants.adaptMacLabel:
            ...
            MAC_results, NLoSReflections = self.macHussam(logger, simEndTime)
    ...
    def setup_devices(
        ...
        AP_RFBox, UE_RFBox = self.setup_RFEquipment(
            beamWidth, UE_BeamWidth, UE_Power, AP_Power, frequency
    ...
    def setup_RFEquipment(self, beamWidth, UE_BeamWidth, UE_Power, AP_Power, frequency):
        ...
        apgain = RF.define_gain(beamWidth)
        uegain = RF.define_gain(UE_BeamWidth)
        # gainUE = RF.define_gain(3)
        AP_RFBox = RF.RFBox(apgain, AP_Power, frequency, beamWidth, self.bandwidth)
        UE_RFBox = RF.RFBox(uegain, UE_Power, frequency, beamWidth, self.bandwidth)
    ...
    def packetEncoding(self):
        ...
        payload_length_encoded = (
            math.ceil(data_packet_length / MAC_Controller.FEC_NUMERATOR) * MAC_Controller.FEC_DENOMINATOR
        )
    ...
    def macAdapt(self, logger: Logger, endTime):
        ...
        while True:
            ...
            CTS_PACKET = MACAP.create_CTS_Packet_ADAPT(
        ...
        if linkType == constants.LoS:
            for device in MAC_UEDEVICES:
                ...
                if ue_device_sector == MACAP.currentSector:  # LoS Condition
                    ...
                    MESSAGES_Logging.append(
                        "LoS CTA Received by UE: "
                        + str(device.ue_device.id)
                        + " sitting at sector: "
                        + str(ue_device_sector)
                        + " at time: "
                        + str(device.lastCTA_ArrivalTime)
                        + ", at sector "
                        + str(MACAP.currentSector)
                    )
                    if device.check_Transmission_Capbaility(
                        sectorEndTime, MACAP.currentSector, constants.LoS, MACAP, simRoom, MESSAGES_Logging
                    ):
                            ...
                            if self.mac_protocol == constants.adaptMacLabel:
                                ...
                                if transmissions < device.lastCTA_ArrivalTime:
                                    RTS_PACKET = device.create_RTS_Packet(
                                        constants.LoS, device.lastCTA_ArrivalTime, MACAP.currentSector, last_RTS_Packet
                                    )
                                else:
                                    RTS_PACKET = device.create_RTS_Packet(
                                        constants.LoS, transmissions, MACAP.currentSector, last_RTS_Packet
                                    )
                                if self.blockage:
                                ...
                                MESSAGES_Logging.append(
                                    "RTS PACKET created with seq id: " + str(RTS_PACKET.sequence_id)
                                )
                                if self.blockage:
                                    blockage_prob = channel.compute_propabilityLoS_indoorMixed(RTS_PACKET.distance)
                                    threshold_pr = math_toolkit.random_uniform_between(0, 1)
            ...
            for device in MAC_UEDEVICES:  # check_Transmission_Capbaility(self,time_window_right,apSector, linkType):
                ...
                if device.check_Transmission_Capbaility(
                    sectorEndTime, MACAP.currentSector, constants.NLoS, MACAP, simRoom, MESSAGES_Logging
                ):
                    device.process_CTA_packet(
                        CTA_PACKET
                    )  # Not actually receiving a CTA, but need it to flush the MACUE system-removing this will impact the MAC ue NloS Transmission time
                    MESSAGES_Logging.append(
                        "NLoS For UE ID: " + str(device.ue_device.id) + "and has something to transmit"
                    )
                    MACUE_devices_withTransmission_Request.append(device)
                    UL_Valid_Transmissions = device.ue_device.check_number_packets(sectorEndTime)
                    last_RTS_Packet = None
                    for transmissions in UL_Valid_Transmissions:
                        RTS_PACKET = device.create_RTS_Packet(
                            constants.NLoS, transmissions, MACAP.currentSector, last_RTS_Packet
                        )
            ...
            for device in MACUE_devices_withTransmission_Request:
                for indexer, ue_ID in enumerate(CTS_PACKET.allocatedUEID):
                    if device.ue_device.id == ue_ID:
                        UL_PACKET, NLoS_Signal = device.process_CTS_packet(
                            CTS_PACKET, sector_start_time, currentSector, Sector_Time
                        )
                        ...
                        for index, pck in enumerate(UL_PACKET):
                            ...
                            MESSAGES_Logging.append(
                                "UEID: " + str(ue_ID) + "generated the following ul packet: " + str(pck.sequence_id)
                            )
                        break
        ...
        for mac_ue_device in MACUE_Devices:
            for rts_packet in collided_packets:
                if rts_packet.sender == mac_ue_device.ue_device.id:
                    ...
                    wait_time_before_collision_awarness = (
                        2 * rts_packet.transmissionDelay + 2 * rts_packet.propagationDelay
                    )
                    transmission_time_of_rts = rts_packet.timeStampTransmission
                    new_transmission_time_of_rts = (
                        transmission_time_of_rts
                        + wait_time_before_collision_awarness
                        + mac_ue_device.compute_RANDOMBACKOFF_time()
                    )
    ...
    def macHussam(self, logger: Logger, endTime):
        ...
        while True:
            ...
            MACUE_devices_withTransmission_Request = (
                MACUE_devices_withTransmission_Request_LoS + MACUE_devices_withTransmission_Request_NLoS
            )
            ...
            MESSAGES_Logging.append(
                "Total RTS Packets: " + str(len(RTS_PACKETS)) + ", Dropped RTS packets: " + str(len(RTS_DROPPED))
            )
            ...
            CTS_PACKET = self.setup_CTS_packets(RTS_PACKETS, MESSAGES_Logging, MACAP)
            ...
            for MACUE_device in MACUE_devices_withTransmission_Request:
                for ACK_Packet in ACK_Packets:
                    if MACUE_device.ue_device.id == ACK_Packet.ueIDlist[0]:
                        ...
                        MESSAGES_Logging.append(
                            "Tput for transaction: " + str((self.dataPacketLength_Encoded / latency) / 1e9)
                        )
                        if latency < 0:
