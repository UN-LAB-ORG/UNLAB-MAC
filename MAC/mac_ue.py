...
class macUE:
    ...
    def check_Transmission_Capbaility(self, endTime, apSector, linkType, MACAP, simRoom, MESSAGES_Logging):
        ...
        if self.ue_device.check_for_transmission(endTime):  # self.lastCTA_ArrivalTime+time_window_right)):
            if linkType == constants.LoS:
                ...
                if self.mySector != apSector:
                    if (
                        self.NLoS_Signal[MACAP.currentSector] == None
                        or self.NLoS_Signal[MACAP.currentSector].NLoS == 0
                    ):
    ...
    def compute_RANDOMBACKOFF_time(self):
        return math_toolkit.random_uniform_between(macUE.RANDOM_BACKOFF_MINTIME, macUE.RANDOM_BACKOFF_MAXTIME) + (
            math_toolkit.random_uniform_between(0, 2**self.retransmissions) * 1e-9
        )

    def create_RTS_Packet(
        self, linkType, transmissionInstance, currentSector, prevRTSPacket
    ):  # UL_timeTable for MACHUSSAM -> need to integrate into ADAPT
        timeAdvance = 0
        ...
        if prevRTSPacket != None:
            if transmissionInstance <= (
                prevRTSPacket.timeStampTransmission + prevRTSPacket.transmissionDelay + small_time_delta
            ):
                timeAdvance = (
                    prevRTSPacket.timeStampTransmission + prevRTSPacket.transmissionDelay + small_time_delta
        ...
        if linkType == constants.LoS:
            ...
            if self.ue_device.RFBox.splitBandwidthValid:
                p_rx, max_data_rate, ber, modulation_scheme = channel.link_budget(
                    self.ue_device.RFBox.power,
                    self.distanceToAP,
    ...
    def setupNLoSLinks_helper(self, AP, currentSector, simRoom):
        ...
        if my_mirrors:
            for mirror in my_mirrors:
                ...
                incidence_distane = math_toolkit.euclidean_distance(
                    self.ue_device.xCor, self.ue_device.yCor, NLoS_Signal.reflect_x, NLoS_Signal.reflect_y
                )
                reflection_distance = math_toolkit.euclidean_distance(
                    NLoS_Signal.reflect_x, NLoS_Signal.reflect_y, AP.xCor, AP.yCor
                )
    ...
    def OMNIMAC_createULGrant(self, timeSlots):
        ...
        if (
            start_index == -1
        ):  # my earliest transmission is beyond that of the time slots available. I do not need to send.
    ...
    def process_CTA_packet(self, CTA: CTA):
        arrival_time = (
            CTA.timeStampTransmission + channel.compute_propagationDelay(self.distanceToAP) + CTA.transmissionDelay
        )
    ...
    def process_CTS_packet(self, CTS: CTS, sectorStartTime, currentSector, sectorTime):
        ...
        if self.mySector != currentSector:
            ...
            arrival_time = (
                CTS.timeStampTransmission + channel.compute_propagationDelay(NLoS_Distance) + CTS.transmissionDelay
            )
            CTS.setupPropagationDelay(NLoS_Distance, self.ue_device.id)
        else:
            arrival_time = (
                CTS.timeStampTransmission + channel.compute_propagationDelay(self.distanceToAP) + CTS.transmissionDelay
            )
        ...
        for i in range(len(CTS.allocatedTimeSlots)):
            if CTS.allocatedUEID[i] == self.ue_device.id:
                ...
                if find_time_slot < self.ue_device.UE_TRANSMISSIONS.check_earliest_transmission():
                    ...
                    sys.exit(1)
                sector_forTransmission = self.ue_device.AP.find_current_sector(
                    sectorStartTime, currentSector, sectorTime, find_time_slot
                )
                linkType = None
                ...
                UL_PACKET, NLoS_Signal = self.create_ULDATA_Packet(
                    linkType, sector_forTransmission, find_time_slot, data_rate_approved
                )
    ...
    def process_ACK_packet(self, ACKs: ACK, currentSector):
        ...
        for index, device_id in enumerate(ACKs.ueIDlist):
            if device_id == self.ue_device.id:
                arrival_time = (
                    ACKs.timeStampTransmission
                    + channel.compute_propagationDelay(self.distanceToAP)
                    + ACKs.transmissionDelay
                )
                ACKs.settimeStampArrival(arrival_time)
                ACKs.setupPropagationDelay(self.distanceToAP, self.ue_device.id)
                timeOfPacketCreation, ul_packet = self.ue_device.transmission_succesful(
                    ACKs.packetsACKED_UELIST[index], currentSector
                )
    ...
    def process_ACK_packet_NLoS(self, ACKs: ACK, APSector):
        ...
        arrival_time = (
            ACKs.timeStampTransmission + channel.compute_propagationDelay(ack_distance) + ACKs.transmissionDelay
        )
        latency = arrival_time - timeOfPacketCreation
    ...
    def link_budget_RTS(ue_device: UE.UE, AP: AP.AP):
        ...
        p_rx, max_data_rate, BER, modulation_scheme = channel.link_budget(
            ue_device.p_tx, distance, AP.bandwidth, ue_device.txGain + AP.rxGain, AP.f_c_UL, 0
        )
    ...
    def requestMSC(UE_device: UE.UE, RTS_PACKET: RTS, ue_to_ap_distance):
        p_rx, max_data_rate, ber, modulation_scheme = channel.link_budget(
            UE_device.p_tx,
            ue_to_ap_distance,
            UE_device.AP.maxBandwidth,
            UE_device.txGain + UE_device.AP.rxGain,
            UE_device.AP.f_c_UL,
            0,
        )
    ...
    def process_CTS_packet_ADAPT(self, CTS: CTS, currentSector):
        ...
        for i in range(len(CTS.allocatedTimeSlots)):
            if CTS.allocatedUEID[i] == self.ue_device.id:
                ...
                linkType = constants.LoS
                UL_PACKET, NLoS_Signal = self.create_ULDATA_Packet(
                    linkType, currentSector, find_time_slot, data_rate_approved
                )
        return UL_PACKET
