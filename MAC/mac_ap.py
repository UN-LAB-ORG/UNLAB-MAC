...
class macAP:
    ...
    def maxControlDelay(self, packet_length, distance, bit_rate, processing_time):
        return (
            channel.compute_propagationDelay(distance)
            + channel.compute_transmissionTime(packet_length, bit_rate)
            + processing_time
        )
    ...
    def create_CTS_Packet_ADAPT(self, packets: RTS, maxDistance, currentTime, endTime=None):
        ...
        transmission_timeSlot_advance = currentTime + channel.compute_transmissionTime(
            CTS_Packet.CONTROL_PACKET_LENGTH, CTS_Packet.CONTROL_PACKET_RATE
        )
        ...
        for indexer, requests in enumerate(timeStamp_forRequest):
            ...
            if time_slot_assigned < requests:
                ...
                sys.exit(1)
            CTS_Packet.setupTimeSlots(
                time_slot_assigned, ue_recepient, dataRates_requested[indexer], linkType_assigned
            )
            counter += 1
        ...
        if len(sortedPackets) == 0:
            ...
            self.lastULArrival_endTime = (
                transmission_timeSlot_advance  # This will be used as the start time of the ACK transmission.
            )
    ...
    def create_CTS_Packet_OMNI(self, packets: RTS):  ## the endtime needs to account for the ACK...
        ...
        CTS_Packet.setupTransmissionDelay()
        CTS_Packet.settimeStampTransmission(
            packets_sorted[0].timeStampArrival
        )  # RTS cutoff time. does not accound for the prop delay of the last rTS but its insignificant (ignore lvl high)
        packet_transmission_time = 0
        ...
        for indexer, packet in enumerate(packets_sorted):
            ...
            earliest_grant_Time = (
                packet.timeStampArrival
                + packet.propagationDelay
                + channel.compute_transmissionTime(CTS_Packet.CONTROL_PACKET_LENGTH, CTS_Packet.CONTROL_PACKET_RATE)
            )
            for grant_index in range(0, numOfGrantsRequested):
                timeSqueeze = self.find_earliest_available_slot(
                    packet_transmission_time + prop_delay_ap_to_user, earliest_grant_Time
                )
                if timeSqueeze != None:
                    ...
                    self.ULGrantsAllocationTable_OMNI_end.append(
                        transmission_timeSlot + packet_transmission_time + prop_delay_ap_to_user
                    )
                time_slot_assigned = transmission_timeSlot
                linkType_assigned = packet.linkType
                CTS_Packet.setupTimeSlots(
                    time_slot_assigned, ue_recepient, packet.computed_data_rate, linkType_assigned
                )
        CTS_Packet.setup_recepients()
        return CTS_Packet
    ...
    def plot_time_slots(self, rts_arrival_time, assigned_start, assigned_end):
        ...
        for i, (start, end) in enumerate(
            zip(self.ULGrantsAllocationTable_OMNI_start, self.ULGrantsAllocationTable_OMNI_end)
        ):
            width = end - start
            ...
            ax.add_patch(
                patches.Rectangle(
                    ...
                    facecolor=color,
                    alpha=0.7,
                    label=f"Slot {i+1}" if color == "blue" else "Assigned Slot",
                )
            )
        ...
        ax.set_ylim(0, 1.5)
        ax.set_xlim(
            0.001, max(self.ULGrantsAllocationTable_OMNI_end) + (max(self.ULGrantsAllocationTable_OMNI_end) * 0.1)
        )
        ax.set_xlim(
            self.ULGrantsAllocationTable_OMNI_end[1],
            max(self.ULGrantsAllocationTable_OMNI_end) + (max(self.ULGrantsAllocationTable_OMNI_end) * 0.1),
        )
        ax.set_xlabel("Time")
        ...
        ax.legend(
            handles=[
                patches.Patch(color="blue", label="Other Slots"),
                patches.Patch(color="red", label="Assigned Slot"),
            ],
            loc="upper right",
        )
