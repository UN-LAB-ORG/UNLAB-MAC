...
class UETransmission:
    ...
    def __init__(self, max_reTransmissions):
        ...
        self.transmission_IDList = (
            []
        )  # Generates a internal id for every application layer transmission instance to track packets
    ...
    def transmit_failure(self, ulPacketSequencID, linkType, APSector):
        ...
        if match == -1:
            ...
            print(
                "Processing a Failed Transmission Failed. UL Packet does not exist anymore <Two Failures - look at you>"
            )
            sys.exit(1)
...
class UE:
    ...
    def setupUE(self):
        UE_Transmission_Time = self.setup_trasnmission_time(
            start_time=self.startTime, end_time=self.simEndTime, lambda_=self.transmission_rate
        )
    ...
    def check_for_transmission(self, current_time):
        ...
        if (
            not (self.check_transmission_queue())
            and self.UE_TRANSMISSIONS.check_earliest_transmission() <= current_time
        ):
            return True
        return False
    ...
    def UL_Transmit(self):
        ...
        if self.LoS == 1:
            ...
            distance = math_toolkit.euclidean_distance(
                self.xCor, self.yCor, self.reflect_x[index], self.reflect_y[index]
            ) + math_toolkit.euclidean_distance(self.reflect_x[index], self.reflect_y[index], self.AP.x, self.AP.y)
        else:
            print("ERROR: UL Transmit Intiated, but NO LOS or NLOS is available. ")
        return channel.link_budget(
            self.p_tx, distance, self.AP.maxBandwidth, self.txGain + self.AP.rxGain, self.AP.f_c_UL, 0
        )
