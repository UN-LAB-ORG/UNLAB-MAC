...
dataPath = load_latest_transmission_logs(os.path.join("Logs", "PacketTrace"), "transmission_logs.csv")
...
class LogViewerApp:
    def __init__(self, root):
        ...
        self.table = ttk.Treeview(
            table_frame, columns=columns, show="headings", height=10, yscrollcommand=self.v_scroll.set
        )
        ...
        result_plotPaths = os.path.join("Results", get_lastFolder("Results"), "Individual_UE_RESULTS")
        ...
        self.transmission_table_data = ttk.Treeview(
        ...
        self.ue_ids = senders
        self.ue_id_table = ttk.Treeview(
            self.ue_id_frame,
            columns=["UE ID", "Sector", "Distance To AP [m]", "Propagation Delay [ns]"],
            show="headings",
            height=10,
        )
        ...
        for index, ue_id in enumerate(self.ue_ids):
            if ue_id == 0:
                continue
            sector_path_ueid = load_latest_transmission_logs(
                os.path.join("Logs", "UE_LOG"), "UE_" + str(ue_id) + ".txt"
            )
    ...
    def load_plot_image(self):
        ...
        path = load_latest_transmission_logs("Results", os.path.join("Room", "Room_Setup.png"))
    ...
    def on_ue_id_click(self, event):
        ...
        file_path = load_latest_transmission_logs(os.path.join("Logs", "UE_LOG"), "UE_" + str(ue_id_value) + ".txt")
        ...
        with open(file_path, "r") as file:
            ...
            if id_line_pattern in file_content:
                ...
                if sector_start_pos != -1:
                    ...
                    distance = round(
                        float(
                            (file_content[file_content.find("DistanceToAP") :].split("=")[1].split()[0]).replace(
                                ",", ""
                            )
                        ),
                        3,
    ...
    def on_transmission_row_double_click(self, event):
        ...
        pickleFilePath = os.path.join(pickleFilePath, get_lastFolder(pickleFilePath), "NLoSData", "NLoSReflection.pkl")
        ...
        pickleFilePath_1 = os.path.join(
            imageFilePath, get_lastFolder(imageFilePath), "Room", "mirrorFoV", str(self.lastUESelected)
        )
        pickleFilePath_2 = os.path.join(
            imageFilePath,
            get_lastFolder(imageFilePath),
            "Room",
            "mirrorRoom",
            str(self.lastUESelected),
            "mirrorRoom.pkl",
        )
        pickleFilePath_3 = os.path.join(
            imageFilePath,
            get_lastFolder(imageFilePath),
            "Room",
            "NLoSAllSignals",
        ...
        mirror_subfolders = [
            f for f in os.listdir(pickleFilePath_1) if os.path.isdir(os.path.join(pickleFilePath_1, f))
        ]

        # Helper function to update the leftmost figure when dropdown changes
        def update_leftmost_image(selected_folder):
            pickleFilePath_1_new = os.path.join(
                imageFilePath, get_lastFolder(imageFilePath), "Room", "mirrorFoV", str(self.lastUESelected)
            )
        ...
        dropdown = ttk.OptionMenu(
            plot_window, selected_option, None, *mirror_subfolders, command=update_leftmost_image
        )
            ...
            try:
                if i == 1:
                    with open(path, "rb") as file:
                        ...
                        ax = fig_new.gca()
                        if linkType == "NLoS":
                            plotter.plot_single_UE_links2(
                                ax, RFLoS_Signal, self.ue_mappingCordinates[self.lastUESelected], 0
                            )
