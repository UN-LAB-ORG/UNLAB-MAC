...
class Reflector:
    def __init__(
        self, reflect_x, reflect_y, reflect_slope, reflect_intercept, incident_slope, incident_intercept, NLoS
    ):
...
class room2:
    ...
    def setup_mirrors_in_room(self, mirror):
        if mirror == None:
            self.MIRRORS = None
            return
        self.MIRRORS = mirror
        for sector in self.MIRRORS:
            for i in range(0, len(self.MIRRORS[sector])):
                self.MIRRORS[sector][i].xCorP1 = self.MIRRORS[sector][i].c_x - self.MIRRORS[sector][
                    i
                ].length / 2 * math.cos(math.radians(self.MIRRORS[sector][i].angleTilt))
                self.MIRRORS[sector][i].yCorP1 = self.MIRRORS[sector][i].c_y - self.MIRRORS[sector][
                    i
                ].length / 2 * math.sin(math.radians(self.MIRRORS[sector][i].angleTilt))
                self.MIRRORS[sector][i].xCorP2 = self.MIRRORS[sector][i].c_x + self.MIRRORS[sector][
                    i
                ].length / 2 * math.cos(math.radians(self.MIRRORS[sector][i].angleTilt))
                self.MIRRORS[sector][i].yCorP2 = self.MIRRORS[sector][i].c_y + self.MIRRORS[sector][
                    i
                ].length / 2 * math.sin(math.radians(self.MIRRORS[sector][i].angleTilt))
    ...
    def setup_fov_generic(self, AP):
        ...
        for sector in self.MIRRORS:
            for i in range(len(self.MIRRORS[sector])):
                ...
                fov_1_y = (
                    m_1 * (np.array(fov_1_x) - self.MIRRORS[sector][i].xCorP1) + self.MIRRORS[sector][i].yCorP1
                ).tolist()
                fov_2_y = (
                    m_2 * (np.array(fov_2_x) - self.MIRRORS[sector][i].xCorP2) + self.MIRRORS[sector][i].yCorP2
                ).tolist()
    ...
    def setup_valid_reflection_vectors(self, UE, AP, current_sector, mirror):
        ...
        NLOS = 0
        NLoS_Signal = Reflector(
            reflect_x, reflect_y, reflect_slope, reflect_intercept, incident_slope, incident_intercept, NLOS
        )
        for s in boundary_points:
            ...
            slope_left, intercept_left = math_toolkit.convert_vector_to_slope(
                left_vector, (AP.x, AP.y), 100, 0, end_1 * self.length
            )
            slope_right, intercept_right = math_toolkit.convert_vector_to_slope(
                right_vector, (AP.x, AP.y), 100, 0, end_1 * self.length
            )

            if slope_left == None:
                degree_org = AP.sector_leftBoundary[int(current_sector)][2] + 1
                left_vector = (
                    round(math.sin(math.radians(degree_org)), 7),
            ...
            if slope_right == None:
                ...
                slope_right, intercept_left = math_toolkit.convert_vector_to_slope(
                    right_vector, (AP.x, AP.y), 100, 0, end_1 * self.length
                )
            ...
            if slope != np.inf and (valid_reflection):
                ...
                if np.isclose(y_at_observer, y_o, atol=1):
                    if NLOS <= 0 or abs(y_at_observer) < abs(reflect_intercept):
                        ...
                        break
            elif slope == np.inf and (
                AP.sector_leftBoundary[int(current_sector)][2]
                <= 360 - math.degrees(math.atan(slope))
                <= AP.sector_rightBoundary[int(current_sector)][2]
            ):
    ...
    def getUESector(
        self, UE, AP
    ):  # Finds in which sector the UE is currenty sitting in. If the UE is mobile, it will have to update its X,Y coordiantes before calling this function
        x = UE.xCor
        y = UE.yCor
    ...
    def mirrors_with_coverage(self, UE, currentSector):
        mirrors_UEFoV_Pass = (
            []
        )  # holds the mirrors the satistfy the condition of UE coordinates are bounded by the mirror FoV
    ...
    def setup_all_reflection_vectors(
        self, ueDevice, AP, currentSector, custom_single_link_per_plot=False, index_mirror=-1
    ):
        x_l = ueDevice.xCor
        y_l = ueDevice.yCor
        x_o = AP.x
        ...
        for i in range(len(my_mirrors)):
            ...
            self_NLOS = 0
            NLoS_Signal = Reflector(
                reflect_x, reflect_y, reflect_slope, reflect_intercept, incident_slope, incident_intercept, self_NLOS
            )
            for s in boundary_points:
                # Compute the boundary point
                x_b = x1 + s * (x2 - x1)
