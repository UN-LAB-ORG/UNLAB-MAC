...
def plot_mirrors(plt, mirrors, mirror_index=-1):
    ...
    for i in range(len(mirrors)):
        plt.plot(
            [mirrors[i].xCorP1, mirrors[i].xCorP2], [mirrors[i].yCorP1, mirrors[i].yCorP2], colors[i % len(colors)]
        )
    # if(mirror_index == -1):
...
def plot_ALL_APSector(plt, AP, room, num_points):
    ...
    for current_sector in AP.sector_map:
        ...
        left_x, left_y = math_toolkit.convert_vector_to_line(
            left_vector, (AP.x, AP.y), num_points, 0, end_1 * room.length
        )
        right_x, right_y = math_toolkit.convert_vector_to_line(
            right_vector, (AP.x, AP.y), num_points, 0, end_2 * room.length
        )
        if left_y == None:
            y_direction_vertical = (
                -1
                if (
                    AP.sector_leftBoundary[current_sector - 1][2] > 90
                    and AP.sector_leftBoundary[current_sector - 1][2] < 270
                )
                else 1
            )
            ...
            y_direction_vertical = (
                -1
                if (
                    AP.sector_rightBoundary[current_sector - 1][2] > 90
                    and AP.sector_rightBoundary[current_sector - 1][2] < 270
                )
                else 1
            )
...
def plot_APSector(plt, AP, current_sector, room, num_points, color_pick):
    ...
    if left_y == None:
        y_direction_vertical = (
            -1
            if (AP.sector_leftBoundary[current_sector][2] > 90 and AP.sector_leftBoundary[current_sector][2] < 270)
            else 1
        )
        triangle_vertices = [(AP.x, AP.y), (AP.x, room.length * y_direction_vertical), (right_x[-1], right_y[-1])]
        x, y = zip(*triangle_vertices)
        ...
        y_direction_vertical = (
            -1
            if (AP.sector_rightBoundary[current_sector][2] > 90 and AP.sector_rightBoundary[current_sector][2] < 270)
            else 1
        )
        triangle_vertices = [(AP.x, AP.y), (AP.x, room.length * y_direction_vertical), (left_x[-1], left_y[-1])]
        x, y = zip(*triangle_vertices)
...
def plot_UE_Devices(plt, UE_list):
    for i in range(len(UE_list)):
        ...
        plt.text(
            ue_device.xCor + 0.1, ue_device.yCor, f"UE {ue_device.id}", fontsize=9, color="red"
...
def plot_UE_NLoS(plt, ue_device):
    ...
    plt.plot(
        [ue_device.reflect_x[indexer], 0],
        [ue_device.reflect_y[indexer], ue_device.reflect_intercept[indexer]],
        "green",
    )
...
def plot_UE_NLoS_nonActive(plt, ue_device):
    ...
    for i in range(len(ue_device.mirror_mapping)):
        ...
        line_2x, line_2y = math_toolkit.create_line(
            ue_device.reflect_slope[i], ue_device.reflect_intercept[i], 5, ue_device.reflect_x[i], ue_device.AP.x
        )
        line_1x, line_1y = math_toolkit.create_line(
            ue_device.incident_slope[i], ue_device.incident_intercept[i], 5, ue_device.xCor, ue_device.reflect_x[i]
        )
        # print(line_1x)
        # print(line_1y)
...
def plot_UE_NLoS_nonActive2(plt, ue_device, NLoS_Signals):
    for i in range(len(NLoS_Signals)):
        line_2x, line_2y = math_toolkit.create_line(
            NLoS_Signals[i].reflect_slope,
            NLoS_Signals[i].reflect_intercept,
            5,
            NLoS_Signals[i].reflect_x,
            ue_device.AP.x,
        )
        line_1x, line_1y = math_toolkit.create_line(
            NLoS_Signals[i].incident_slope,
            NLoS_Signals[i].incident_intercept,
            5,
            ue_device.xCor,
            NLoS_Signals[i].reflect_x,
        )
...
def plot_UE_NLoS_Active2(plt, ueXcor, APxCor, NLoS_Signal):
    line_2x, line_2y = math_toolkit.create_line(
        NLoS_Signal.reflect_slope, NLoS_Signal.reflect_intercept, 5, NLoS_Signal.reflect_x, APxCor
    )
    line_1x, line_1y = math_toolkit.create_line(
        NLoS_Signal.incident_slope, NLoS_Signal.incident_intercept, 5, ueXcor, NLoS_Signal.reflect_x
    )
    plt.plot(line_1x, line_1y, "green")
    plt.plot(line_2x, line_2y, "green")
    return plt
...
def statistics_plot_sectorUsage(Sector_activity_Data_Uplink):
    ...
    padded_data = [
        Sector_activity_Data_Uplink[data] + [0] * (max_instances - len(Sector_activity_Data_Uplink[data]))
        for data in Sector_activity_Data_Uplink
    ]
