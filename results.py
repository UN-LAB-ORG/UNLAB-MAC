...
class Results:
    ...
    def setup_ue_perf(self, ue_id, plotName, data, xlabel, ylabel, title, legend_label):
        ...
        np.savetxt(
            sub_sub_sub_folder + "\\data.txt",
            data,
            fmt="%d",
        )  # Change fmt for desired precision
    ...
    def process_results_generic(self, macResults, PAYLOADSIZE, showPlots=False, savePlots=True):
        ...
        for uid, data in grouped_data.items():
            ...
            self.setup_ue_perf(
                ...
                "Measurement Index",
                "Data Rate (Gbps)",
                f"Data Rates for User {uid}",
                f"User {uid} Data Rate",
            )
            self.setup_ue_perf(
                uid,
                "Throughput",
                ...
                "Measurement Index",
                "Throughput (Gbps)",
                f"Throughput for User {uid}",
                f"User {uid} Throughput",
            )
        ...
        for uid, data in grouped_data.items():
            ...
            avg_throughput_across_all = (
                avg_throughput_across_all + (PAYLOADSIZE / np.array(data["latencies"])).tolist()
            )
        ...
        avg_latency_hist = plotter.results_create_histogram_plot(
            uid_list,
            avg_latencies,
            "Measurement Index",
            "Average Latency (ms)",
            "Histogram of Average Latency Across All Users",
            latency_file_path,
        )
        avg_data_rate_hist = plotter.results_create_histogram_plot(
            uid_list,
            avg_data_rates,
            "Measurement Index",
            "Average Data Rate (Gbps)",
            "Histogram of Average Data Rate Across All Users",
            data_file_path,
        )
        avg_throughput_hist = plotter.results_create_histogram_plot(
            uid_list,
            avg_throughputs,
            "Measurement Index",
            "Average Throughput (Gbps)",
            "Histogram of Average Throughput Across All Users",
            thruput_file_path,
        )
