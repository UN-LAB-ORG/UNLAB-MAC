...
class MAC_Controller:
    ...
    def collision_detection_ul2(self, packets, MESSAGES_Logging):
        ...
        for packet in packets:
            ...
            if success:
                packets_success.append(packet)
                MESSAGES_Logging.append(
                    "Packet " + packet.packetType + "with seq id: " + str(packet.sequence_id) + " occured no collision"
                )
                ...
                MESSAGES_Logging.append(
                    ...
                    + ", "
                    + str(next_packet.sequence_id)
                    + "has been dropped due to collision"
                )
                MESSAGES_Logging.append(
                    "UE device: "
                    + str(packet.sender)
                    + ", and UE Device: "
                    + str(next_packet.sender)
                    + " sent Packets that collided with one another"
                )
                MESSAGES_Logging.append(
                    "First Packet Transmission and Arrival Times are: "
                    + str(packet.timeStampTransmission)
                    + ", "
                    + str(packet.timeStampArrival)
                )
                MESSAGES_Logging.append(
                    "First Packet Transmission and Arrival Times are: "
                    + str(next_packet.timeStampTransmission)
                    + ", "
                    + str(next_packet.timeStampArrival)
                )
        return packets_dropped, packets_success
    ...
    def setup_devices(
        ...
        startTime,
        simEndTime,
        fixed_ue_coordinates,
    ):
        # Top Function to Setup the UE Device handset and AP
        AP_RFBox, UE_RFBox = self.setup_RFEquipment(
            beamWidth, UE_Power, AP_Power, frequency
        ...
        if numAP > 1:
            ...
            if ue_coordinates == None:
                ue_coordinates = math_toolkit.randomCoordinates(
                    expected_nodes, (((self.simRoom.length - (MAC_Controller.BOUNDARY_OF_ROOM * self.simRoom.length))))
                )
        ...
        for i, (x, y) in enumerate(ue_coordinates, 1):
            UE_DEVICE = UE.UE(
                x, y, MAC_Controller.UE_MAX_RETRANSMISSION, UE_RFBox, UE_uplinkTransmissionRate, startTime, simEndTime
            )
            UE_DEVICE.setupUE()
    ...
    def los_linkbudget(self):
        ...
        d_support = channel.max_distance(
            self.AP.RFBox.power, self.bandwidth, self.AP.RFBox.gain, self.AP.RFBox.frequency
        )
    ...
    def macAdapt(self, logger: Logger, endTime):
        ...
        while True:
            ...
            PACKETS_Logging = []
            # new Sector: Send CTA
            max_time_elapsed_CTA = (
                MACAP.maxControlDelay(
                    Packet.CONTROL_PACKET_LENGTH,
                    MAC_Controller.MAX_DISTANCE_CONTROL_SUPPORT,
                    Packet.CONTROL_PACKET_RATE,
                    PROCESSING_TIME_PACKET_control,
                )
                / time_scale
            )
            max_time_elapsed_RTS = max_time_elapsed_CTA
            total_wait_time = (
                simulationTotalTimeElapsed + max_time_elapsed_CTA + max_time_elapsed_RTS + RANDOM_BACK_OFF_TIME_MAX
            )
            ...
            for MACUE_device in MACUE_devices_withTransmission_Request:
                ...
                RTS_PACKET = MACUE_device.create_RTS_Packet(
                    "LoS", MACUE_device.ue_device.UE_TRANSMISSIONS.check_earliest_transmission()
                )
                if (
                    RTS_PACKET.timeStampArrival < total_wait_time
                ):  # UE was able to send an RTS before elapsed wait time. After elapsed wait time, AP turns into new sector.
                    # Make sure link can be closed otherwise AP wont recieve it ...
                    if (
                        MACUE_device.distanceToAP <= self.MAX_DISTANCE_CONTROL_SUPPORT
                    ):  # note that i am using the control support here
            ...
            if len(RTS_PACKETS) == 0:  # No Transmission Needed in this in time slot
                simulationTotalTimeElapsed += (
                    max_time_elapsed_CTA
                    + max_time_elapsed_RTS
                    + RANDOM_BACK_OFF_TIME_MAX
                    + MAC_Controller.SectorTransitionTimeDelay
                )
                apSector += 1
            ...
            CTS_PACKET = MACAP.create_CTS_Packet(
                RTS_PACKETS, MAC_Controller.MAX_DISTANCE_CONTROL_SUPPORT, total_wait_time
            )
            ...
            if len(UL_DROPPED) > 0:
                ...
                collision_detection_tester.plot_packets(
                    UL_PACKETS, UL_DROPPED, UL_SUCCESS, UL_PACKETS[0].timeStampTransmission + 0.005, 20
                )
            ...
            simulationTotalTimeElapsed += (
                ACK_PACKETS.timeStampTransmission - simulationTotalTimeElapsed
            ) + MAC_Controller.SectorTransitionTimeDelay
            apSector += 1
            if apSector > self.AP.number_of_sectors:
                apSector = 0
    ...
    def macHussam(self, logger: Logger, endTime):
        ...
        while True:
            ...
            devices_done = 0
            MESSAGES_Logging.append(
                "RTS END TIME is set to : " + str(sector_start_time + (LoS_Sector_Time * LoS_RTS_Time_Percentage))
            )
            for device in MAC_UEDEVICES:  # check_Transmission_Capbaility(self,time_window_right,apSector, linkType):
                if device.return_UE_Sector() == MACAP.currentSector:
                    CTA_PACKET.addRecepients(device.ue_device.id)
                    ...
                    if device.check_Transmission_Capbaility(
                        sector_start_time + UE_RTS_ENDTIME, MACAP.currentSector, "LoS"
                    ):
                        MESSAGES_Logging.append("UE has something to transmit")
                        MACUE_devices_withTransmission_Request.append(device)
                        UL_Valid_Transmissions = device.ue_device.check_number_packets(
                            sector_start_time + UE_RTS_ENDTIME
                        )
                        MESSAGES_Logging.append(
                            "Packet Time slots that pass the time criteria: "
                            + str(" ".join(map(str, UL_Valid_Transmissions)))
                        )
                        RTS_PACKET = device.create_RTS_Packet(
                            "LoS", sector_start_time + UE_RTS_ENDTIME, UL_Valid_Transmissions
                        )
            ...
            MESSAGES_Logging.append(
                "Total RTS Packets: " + str(len(RTS_PACKETS)) + ", Dropped RTS packets: " + str(RTS_DROPPED)
            )
            RTS_Failures += len(RTS_DROPPED)
            Total_RTS += len(RTS_PACKETS)
            ...
            for dropped_packet in RTS_DROPPED:
                for device in MACUE_devices_withTransmission_Request:
                    if dropped_packet.sender == device.ue_device.id and (
                        not (dropped_packet.sender in already_processed_ue)
                    ):
                        device.process_RTS_collision()
            ...
            if len(RTS_PACKETS) > 0:
                ...
                CTS_PACKET = MACAP.create_CTS_Packet(
                    RTS_PACKETS,
                    MAC_Controller.MAX_DISTANCE_CONTROL_SUPPORT,
                    sector_start_time + (LoS_Sector_Time * LoS_RTS_Time_Percentage),
                    sector_start_time + LoS_Sector_Time,
                )
                MESSAGES_Logging.append(
                    "The UL Start Time is set to be: "
                    + str(sector_start_time + (LoS_Sector_Time * LoS_RTS_Time_Percentage))
                )
                ...
                for indexer, grantsApproved in enumerate(CTS_PACKET.allocatedTimeSlots):
                    ...
                    MESSAGES_Logging.append(
                        "UE : " + str(ueID) + ", has been alloacted the following time slot: " + str(timeSlot)
                    )
                    ...
                    for device in MACUE_devices_withTransmission_Request:
                        if device.ue_device.id == ue_ID:
                            ...
                            MESSAGES_Logging.append(
                                "UEID: "
                                + str(ue_ID)
                                + "generated the following ul packet: "
                                + str(UL_PACKET.sequence_id)
                            )
            ...
            for MACUE_device in MACUE_devices_withTransmission_Request:
                for ACK_Packet in ACK_Packets:
                    if MACUE_device.ue_device.id == ACK_Packet.ueIDlist[0]:
                        ...
                        latency, data_rate = MACUE_device.process_ACK_packet_NLoS(
                            ACK_Packet, "LoS", MACAP.currentSector
                        )
                        if latency != None and data_rate != None:
                            RESULT_LATENCY.append(latency)
                            RESULT_DATA_RATE.append(data_rate)
            ...
            for device in MAC_UEDEVICES:  # check_Transmission_Capbaility(self,time_window_right,apSector, linkType):
                ...
                if device.check_Transmission_Capbaility(nLoS_end_time, MACAP.currentSector, "nLoS"):
                    ...
                    NLoS_Signal_highest, max_data_rate_highest, distance = device.setupNLoSLinks(
                        MACAP.AP, MACAP.currentSector, self.simRoom
                    )

                    if (
                        NLoS_Signal_highest == None
                        or len(UL_Valid_Transmissions) == 0
                        or NLoS_Signal_highest.NLoS == 0
                    ):
                        continue
                    # else:
                    ...
                    for index, UL_Valid_Transmission in enumerate(UL_Valid_Transmissions):
                        ...
                        if UL_Valid_Transmission < NLoS_device_endtime:
                            ...
                            if UL_Packet_Transmission_Time < nLoS_end_time:
                                UL_Packet = device.create_ULDATA_Packet_NLoS(
                                    UL_Packet_Transmission_Time, max_data_rate_highest, distance
                                )
                                time_delta += (
                                    UL_Packet.transmissionDelay
                                    + UL_Packet.propagationDelay
                                    + math_toolkit.random_uniform_between(0, 4e-6)
                                )
                ...
                for MACUE_device in MACUE_devices_withTransmission_Request_NLoS:
                    for ACK_PACKET_NLoS in ACK_PACKETS_NLoS:
                        if MACUE_device.ue_device.id == ACK_PACKET_NLoS.ueIDlist[0]:
                            MESSAGES_Logging.append(
                                "UEID : " + str(MACUE_device.ue_device.id) + "has receieved an ACK"
                            )
                            latency, data_rate = MACUE_device.process_ACK_packet_NLoS(
                                ACK_PACKET_NLoS, "NLoS", MACAP.currentSector
                            )
                            if latency != None and data_rate != None:
                                RESULT_LATENCY.append(latency)
                                RESULT_DATA_RATE.append(data_rate)
