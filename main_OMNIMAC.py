...
mirrors = (
    left_vertical_mirror_map | right_vertical_mirror_map | upper_horizontal_mirror_map | lower_horizontal_mirror_map
)
...
system_time = [x * time_scale for x in range(0, 15)]
...
MACSIMULATION.UERandomBackOffTime = 0.1e-6
...
if plot_avg_tput:
    ...
    for i, iat in enumerate(inter_arrival_time):
        ...
        MAC_Results, NLoSReflections = MACSIMULATION.setupMAC(
            number_AP,
            UE_Device_Density,
            AP_BeamWidth,
            UE_BeamWidth,
    ...
    plotter.results_create_line_plot(
        inter_arrival_time,
        avg_tput_data,
        "Inter-Arrival Time [us]",
        "Avg. Tput [Gbps]",
        "Tput Fixed Node Density 0.05 nodes/m^2",
        None,
        "C:\\Users\\Hussam\\Desktop\\MAC_Simulaotr\\OMNIResults",
    )
else:
    MAC_Results, NLoSReflections = MACSIMULATION.setupMAC(
        number_AP,
